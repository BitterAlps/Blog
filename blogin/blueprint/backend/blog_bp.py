...
@be_blog_bp.route('/deleteCategory/', methods=['GET', 'POST'])
@login_required
@permission_required
def delete_category():
    categoryId = request.form.get('id')
    categoryId = int(categoryId)
    blog_type = BlogType.query.filter_by(id=categoryId).first()
    if blog_type is None:
        return jsonify({'not_null': True})
    if blog_type.counts != 0:
        return jsonify({'not_null': True})
    db.session.delete(blog_type)
    db.session.commit()
