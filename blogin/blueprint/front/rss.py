...
def generate_rss(blogs):
    fg = FeedGenerator()
    fg.title('Blogin')
    fg.description('Blogin是一个个人博客网站，后端使用Flask框架，，前端使用Bootstrap4,主要分享一些编程类的技术文章')
    fg.link(href='https://github.com/BitterAlps')
    fg.id(str(len(blogs)))
    for blog in blogs:
        fe = fg.add_entry()
        ...
        fe.author(name='Blogin', email='1503601623@qq.com')
